	...
	switch( $req_method ) {
		...
		case 'GET':
			...
			*/
			...
			/*
				Now we can send back all the eras
			*/
			
			//	Write the SQL to send back information on all eras
			$get_eras_sql =	"SELECT * FROM era";
			
			if($result = $db_conn->query($get_eras_sql)) {
				
				$eras = array();
				
				while ($result_row = $result->fetch_array(MYSQLI_ASSOC)) {
					array_push($eras, $result_row);
				}
				...
				set_error_response( 0 , "I couldn't get the result for the SQL query ($get_eras_sql)");
				break;
			}
